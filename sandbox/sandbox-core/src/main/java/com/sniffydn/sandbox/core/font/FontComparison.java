...
//        fontList.add("PrestigeEliteStd-Slanted");
    ...
    public static void main(String[] args) {
        ...
        for (File m : mono.listFiles()) {
            ...
            if (count > min && count <= max) {
                ...
                System.out.println("Finished (" + count + ") " + m.getName() + ", Elapsed Time: " + ((System.currentTimeMillis() - time) / 1000));
                time = System.currentTimeMillis();
            }
        }
    }

    private static int offPixelCount = 0;
    private static int onPixelCount = 0;
    private static BufferedImage imgA = null;
    private static BufferedImage imgB = null;
    private static BufferedImage resultImage = null;
    private static Graphics resultGraphics;
    ...
    private static void compareImages(File fileA, File fileB, File resultFile) {
        try {
            ...
            final int t1MaxHeight = maxHeight;
...
//                public void run() {
                    Color cA;
                    Color cB;
                    for (int y = 0; y < t1MaxHeight; y++) {
                        for (int x = 0; x < t1MaxHeight; x++) {
